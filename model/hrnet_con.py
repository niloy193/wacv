...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.transition3 = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.stage4, SRM_final_channels = self._make_stage(
            self.stage4_cfg, num_channels, multi_scale_output=True)
        
        
        #stage 5
        self.stage5_cfg = cfg['STAGE5']
        ...
        self.transition4 = self._make_transition_layer(
            [i+j for (i, j) in zip(RGB_final_channels, SRM_final_channels)], num_channels)
    ...
    def forward(self, inp):
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        srm_out = self.stage4(x_list)

        x = [torch.cat([rgb_out[i], srm_out[i]], 1) for i in range(self.stage5_cfg['NUM_BRANCHES'])]
